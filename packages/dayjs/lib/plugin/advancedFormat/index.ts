...
export default (o: any, c: { prototype: any }, d: { en: { ordinal: (number: any) => string } }) => {
  // locale needed later
  const proto = c.prototype;
  const oldFormat = proto.format;
  d.en.ordinal = (number: number) => {
    const s = ['th', 'st', 'nd', 'rd'];
    const v = number % 100;
  ...
  proto.format = function (formatStr: string) {
    ...
    const result = str.replace(/\[([^\]]+)]|Q|wo|ww|w|WW|W|zzz|z|gggg|GGGG|Do|X|x|k{1,2}|S/g, (match: string) => {
      switch (match) {
        case 'Q':
