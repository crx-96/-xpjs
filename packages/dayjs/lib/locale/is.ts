...
function resolveTemplate(key: string | any[], number: number, isFuture: any, withoutSuffix: any) {
  ...
  const unitText = (texts as any)[correctedKey];
  const text = unitText[index];
  return key.length === 1 ? text : `%d ${text}`;
}

function relativeTimeFormatter(number: any, withoutSuffix: any, key: any, isFuture: any) {
